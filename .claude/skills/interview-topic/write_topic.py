...
Что делает: валидирует (уровни 2–8, теги из 17 концептов и 1–3 на карточку, difficulty ∈ levels,
block/subblock ∈ pool, id уникальны и с префиксом "<pool>-"), пишет pool.yaml (веса блоков ∝ числу карточек)
и карточки через python-frontmatter в нормализованном формате проекта, печатает матрицу «колонка × уровень»
и ячейки тоньше --per-cell. --fresh сносит каталог темы; без него существующие файлы остаются, карточки
...
def validate(spec: dict) -> None:
    ...
    for c in cards:
        cid = c.get("id") or ""
        if not ID_RE.match(cid) or not cid.startswith(pool["id"] + "-"):
            die(f"карточка '{cid}': id должен быть slug с префиксом '{pool['id']}-'")
        if cid in seen:
            die(f"дубль id '{cid}'")
        seen.add(cid)
        ...
        if not 1 <= len(tags) <= 3 or not set(tags) <= TAGS:
            die(f"{cid}: tags — 1–3 из {sorted(TAGS)}, получено {tags}")
