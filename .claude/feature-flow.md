...
|------|-----------|---------|
| Задача | открытый GitHub Issue: `gh issue list --state open` | Источник фич — Issues. **Эпик-issue** (заголовок `[epic]…`) — крупная задача, которую сперва декомпозируют на issue-фичи; такие пропускаются. |
...
- Нет подходящих open issues (все открытые не-эпики уже имеют ветку `feature/<номер>-*`).
