...
| **feature-design** | «спроектируй/добавь фичу» | проектирует фичу → спека в `.claude/features/<slug>.md` (код НЕ пишет) |
| **feature-build** | «реализуй фичу по спеке» | реализует спеку **в ветке `feature/<slug>`**, переиспользует другие скиллы, коммитит на ветке (без пуша) |

## Поток разработки фичи
1. **feature-design** `<идея>` → уточняет требования, пишет спеку в `.claude/features/<slug>.md`.
2. Ревью спеки (человек).
3. **feature-build** → создаёт ветку `feature/<slug>`, реализует по плану (переиспользуя interview-*
   скиллы), прогоняет **interview-verify**, коммитит на ветке. Пуш/PR/merge — решает человек.
