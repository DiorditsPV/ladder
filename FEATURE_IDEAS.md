...
## Идеи
- [ ] topbar-redeclutter: шапка перегружена и сжата по ширине — слишком много flow-элементов в один ряд (трек, прогресс, тулбар, сессия, 5+ кнопок). Разгрузить: сгруппировать/свернуть второстепенное (напр. действия в меню «⋯», перенос части в панель/низ), приоритет — читаемость и воздух. Проконсультироваться с агентом-дизайнером. Референс: ![alt text](image.png) — фронт (App.tsx topbar + styles.css). ПРИМЕЧАНИЕ: правит шапку, которая на main минимальна; основной перегруз — на integration/all-ideas (там 7+ кнопок), так что делать осмысленно поверх интеграционного набора.
