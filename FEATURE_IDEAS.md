...
## Платная версия
Фичи для платного тира. Логика монетизации: free = один интервьюер локально; paid = команда / облако / интеграции.
- [ ] team-workspace: облачные аккаунты + роли (admin/интервьюер), общий банк и сессии команды — backend: auth (JWT/OAuth), мульти-тенант, миграция SQLite→Postgres, RBAC; фронт: логин, переключатель воркспейса. База для всего платного.
- [ ] live-panel: совместное интервью в реальном времени — несколько интервьюеров на одной доске, синхрон оценок/заметок, presence, сведение панельных оценок — backend: websockets; фронт: индикаторы присутствия, merge оценок.
- [ ] ats-integration: интеграция с ATS (Greenhouse/Lever/Huntflow) и календарём — экспорт скоркарт/отчётов в карточку кандидата + импорт пайплайна — backend: коннекторы/вебхуки, OAuth провайдеров.
- [ ] white-label-reports: брендированные отчёты — лого/цвета компании, кастомные секции, batch-экспорт по нескольким кандидатам, защищённая ссылка вместо PDF — расширение src/report.ts + backend share-link.
