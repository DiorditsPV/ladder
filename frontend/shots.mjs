...
//
// !!! УСТАРЕЛ. Скрипт снимает экраны режима интервью (кандидаты, сессии, оценки, итог, отчёт),
// который удалён из продукта: ручки /api/candidates, /api/sessions и селекторы .session__*,
// .cta-done, .finish больше не существуют, поэтому `npm run shots` падает. Переписать вокруг
// чек-листа (главная → доска → drawer → банк → тёмная тема) перед следующей пересъёмкой.
