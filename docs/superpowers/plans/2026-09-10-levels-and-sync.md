...
- `sync` **не** удаляет ноды и не каскадит удаление колонок/уровней: конфиг направления обновляется без побочных удалений; исчезнувшие из файлов seed-ноды прячутся (`hidden=1`), вернувшиеся — разворачиваются (`hidden` пишет только sync);
  при ошибках импорта в пуле проход скрытия пропускается; `/api/graph` отдаёт только `hidden=0`.
