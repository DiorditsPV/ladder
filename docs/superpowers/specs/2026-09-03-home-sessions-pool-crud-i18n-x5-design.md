...
## 6. Дополнение 2026-09-03: колонки направления редактируются (`feature/pool-blocks-editor`)

Решение владельца: колонки и под-колонки — динамические данные направления (добавить, удалить,
переименовать), чтобы завести, например, сценарий интервью для продажника с блоками вне IT.
Перетаскивания на канве нет: порядок задаётся в форме (↑ ↓), доска рисует колонки по данным.

- **Форма направления** (модалка на главной): секция «Колонки» — ряд = цвет (палитра из 8) ·
  название · под-колонки (чипы, «+») · ↑ ↓ ✕, кнопка «+ Колонка». При создании к пресетам
  добавляется «Без пресета — свои колонки» (редактор показывается только в этом варианте; с пресетом
  колонки правятся после создания). При правке — название, описание, колонки.
- **API.** `POST /api/pools` принимает ровно одно из `preset` / `blocks` (иначе 422).
  `PUT /api/pools/{id}` принимает `blocks: [{id?, label, color, subblocks?: [{id?, label}]}]`:
  id новых колонок/под-колонок — транслитерация названия, уникальная в направлении/колонке
  (`-2`, `-3`); вес прежний или 1; валидация та же, что для `pool.yaml` (422).
- **Вопросы при правке.** Удалённая колонка удаляет свои вопросы (UI спрашивает confirm с числом);
  удалённая под-колонка переводит вопросы в общую часть колонки (`subblock = NULL`). Одна транзакция.
- **Не делаем:** перетаскивание на доске, правку весов из UI, смену id существующих колонок.
