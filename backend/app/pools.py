...
порядок и подписи колонок фронт, допустимые block/subblock — импортёр; веса блоков —
данные таксономии (write_topic.py ставит их ∝ числу карточек).
...
def block_weights(pool: PoolCfg) -> Dict[str, int]:
    """Веса блоков как словарь id → weight (порядок блоков сохраняется). Хелпер таксономии."""
